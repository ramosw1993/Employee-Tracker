...
function homeMenu() {
  ...
  const viewDepartments = () => {
    db_1.query(`SELECT d_id AS id, department_name AS name FROM department`, (err, rows) => {
        if (err) {
            console.error(err);
        }
        console.table(rows);
  ...
  const viewRoles = () => {
    db_1.query(`SELECT r_id AS id, title AS title, department_name AS department, salary AS salary FROM employee_role
    LEFT JOIN department ON employee_role.department_id = department.d_id`, (err, rows) => {
        if (err) {
            console.error(err);
        }
        console.table(rows);
  ...
  const viewEmployees = () => {
    db_1.query(`SELECT e.e_id as id, concat(e.first_name,' ', e.last_name) AS employee, e.title AS title, e.salary AS salary, e.department_name AS department, 
    ...
    WHERE m.e_id = e.manager_id`, (err, rows) => {
        if (err) {
            console.error(err);
        }
        console.table(rows);
